...
# Check for expired sessions on every data load
if st.session_state.user_id and st.session_state.data_loaded:
    NeroTimeLogic.check_expired_sessions()

# === Load Firebase if user_id exists ===

# NOTE: CHECK readme.txt for detailed explanation of all session_state variables.
...
if not st.session_state.data_loaded and st.session_state.user_id:
    with st.spinner("Loading..."):
        ...
        st.session_state.data_loaded = True # ensures data does not reload on every refresh
    ...
    with col2:
            ...
            if st.button("Sign In", type="primary", use_container_width=True, key="btn_signin"):
                if login_username and login_password:
                    with st.spinner("Signing in..."):
                        ...
                        result = authenticate_user(login_username, login_password) # use authenticate user function to sign in user
        ...
        with tab_register:
            ...
            if st.button("Create Account", type="primary", use_container_width=True, key="btn_register"):
                if not reg_username: # Input Validation
...
# ==== Stats Bar ====
activities  = NeroTimeLogic.get_activities_data() # GET ALL ACTIVITY DATA
...
st.caption(f"👤 {st.session_state.get('username', st.session_state.user_id)}") # username below the stats bar
st.divider()
